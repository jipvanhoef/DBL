...
def create_network_graph(path):
    ...
    df_time = df_enron_sorted.groupby(['date', 'fromEmail']).agg({'sentiment':['mean'], 'fromEmail': [ 'count']}).reset_index() #making a grouping object to group by date
    ...
    def update_mails(clickData):
        email = clickData['points'][0]['text']
        dff = df_time[df_time['fromEmail'] == email]
        title = '<b>{}<br><b>{}'.format(email, 'amount of sent mails over time')
